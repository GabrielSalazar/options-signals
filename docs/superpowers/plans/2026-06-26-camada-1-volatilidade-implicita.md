...
> **Nota (corrigida após execução):** os testes abaixo monkeypatcham `dp.decodificar_opcao_b3` diretamente, em vez de confiar no decoder real — `decodificar_opcao_b3` deriva o mês da letra do ticker (`C`→março, não junho) e o ano relativo à data corrente, então usar tickers/comentários "ingênuos" (ex.: assumir que `PETRC405` é junho) torna o teste dependente da data de execução e do mapa de letras real. Mockar o decoder mantém o teste determinístico e focado na lógica de filtro/ordenação das funções novas, não na decodificação de ticker (já testada em `options_math.py`).
